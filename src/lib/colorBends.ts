...
export function mountColorBends(
  ...
  const pointerRoot = options.pointerRoot ?? container;
  const trackPointer = (options.mouseInfluence ?? 1) > 0 || (options.parallax ?? 0.5) > 0;
  ...
  // RawShaderMaterial avoids Three's shader chunk rewrite, which was
  // silently blanking this effect on newer three builds.
  const material = new THREE.RawShaderMaterial({
    ...
    uniforms: {
      ...
      uNoise: { value: noise },
      uIterations: { value: Math.max(1, Math.min(5, iterations | 0)) },
